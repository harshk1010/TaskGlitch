...
export default function MetricsBar({ metricsOverride }: { metricsOverride?: Metrics }) {
  ...
  // ✅ FIX: safe formatting
  const safeAvgROI = Number.isFinite(averageROI) ? averageROI.toFixed(1) : '—';
  const safeRevenuePerHour = Number.isFinite(revenuePerHour)
    ? revenuePerHour.toFixed(1)
    : '0.0';
      ...
      <CardContent>
        ...
        >
          ...
          <Stat
            ...
            hint={`Based on Avg ROI (${safeAvgROI}) • Total time ${totalTimeTaken}h`}
          />
        </Box>
      </CardContent>
    </Card>
